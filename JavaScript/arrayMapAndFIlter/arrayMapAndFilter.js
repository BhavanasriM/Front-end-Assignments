...
const movieTitiles = movies.map((movie) => movie.title);
console.log(movieTitiles);

const movieTitlesWithRatings = movies.map((movie) => ({
  title: movie.title,
...
const actionMovies = movies.filter(
  (movie) =>
    movie.genre === "Action" && movie.actors.includes("Leonardo DiCaprio")
);
console.log(actionMovies);

//Chaining:
const chainOpp = movies
  .filter(
    (movie) =>
      movie.genre === "Drama" && movie.actors.includes("Christian Bale")
  )
  .map((movie) => ({
    title: movie.title,
    year: movie.year,
...
console.log(chainOpp);

const chainOpp1 = movies
  .filter(
